...
**Update — schematic exists now**: `hardware/nucleo-adapter/` has a KiCad
schematic (`kicad/adapter.kicad_sch`, generated by `kicad/gen_schematic.py`)
implementing everything described above: reverse-polarity protection,
boost + LDO, shared bidirectional track-current sense, and two full
DRV8300D + 6×CSD16327Q3 driver/bridge/BEMF/current-sense stages (front and
rear). ERCs clean (0 errors) as of this writing. Several values noted
above (boost FB divider, bootstrap caps, DRV8300D MODE/DT bias, shunt
values) are still placeholders pending real characterization — see that
directory's `README.md` for the full list of what's verified-real vs.
placeholder, and what was deliberately simplified vs. RemoraNSR3.0's own
reference circuit. No PCB layout yet.
