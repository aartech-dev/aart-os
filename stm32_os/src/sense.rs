...
//!
//! Each motor gets its own dedicated ADC (option 2 from DESIGN.md section
//! 6.1): a physical virtual-neutral resistor node per motor instead of
//! ESCape32's single-motor dual-ADC averaged reference, so ADC1 and ADC2
//! can sample simultaneously with no interleaving between the two motors.
//!
...
use stm32g4xx_hal::adc::config::{ClockMode, Eoc, ExternalTrigger12, SampleTime, Sequence, TriggerMode};
use stm32g4xx_hal::adc::{AdcClaim, AdcCommon, AdcCommonExt, DynamicAdc};
use stm32g4xx_hal::gpio::{Analog, PA0, PA1, PA3, PA4, PA5, PA6, PB1, PB11, PB12, PB2, PF1};
...
impl SenseIsr for MotorASense {
    ...
    fn configure_neutral(&mut self) {
        self.adc.configure_channel(&self.neutral, Sequence::One, SAMPLE_TIME);
    }
